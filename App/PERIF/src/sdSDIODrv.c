...
uint8_t SD_Detect(void)
{
 ...
 //led_dn(BOARD_LED_XOR);
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIO_SD_DAT3, ENABLE);

  GPIO_InitStructure.GPIO_Pin = SD_DAT3_PORT_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD;
  GPIO_Init(SD_DAT3_PORT, &GPIO_InitStructure);
  if(!GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_11))
    {
	   innerState.flagSDDisappeared  = 1;
	   innerState.flagSDPhysAppeared = 0;
	}
  else if(innerState.flagSDDisappeared)
	   innerState.flagSDPhysAppeared = 1;

  if(innerState.flagSDDisappeared)
...
SD_Error SD_PowerON(void)
{
  ...
  GPIO_ResetBits(GPIOC, GPIO_Pin_7);	 //PWR ON KAY
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_SetClrCardDetect(u8 val)
{
  SD_Error errorstatus = SD_OK;
  ...
  /*!< Send ACMD42 .*/
  SDIO_CmdInitStructure.SDIO_Argument = val;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SD_APP_SET_CLR_CARD_DETECT;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&SDIO_CmdInitStructure);
