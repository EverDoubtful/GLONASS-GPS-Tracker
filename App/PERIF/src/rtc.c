...
char *  readRTCTime(RTC_t *rtc)
{
   rtc_gettime(rtc);
   		...
   		rtcTime.tm_sec   = rtc->sec;
		rtcTime.tm_min   = rtc->min;
		rtcTime.tm_hour  = rtc->hour;
		rtcTime.tm_mday  = rtc->mday;
		rtcTime.tm_mon   = rtc->month-1;
		rtcTime.tm_year  = rtc->year - 1900;
		rtcTime.tm_wday  = rtc->wday;
		rtcTime.tm_isdst = rtc->dst;

   return asctime(&rtcTime);
}

/*******************************************************************************/
u32 getRealTime(void)
{
//	return    ((info.utc.year + 1900 - 1980) << 25)       // Year (info.utc.year = 112)
//			| ((info.utc.mon+1) << 21)       // Month = 
//			| (info.utc.day << 16)       // Day =  
//			| (info.utc.hour << 11)       // Hour 
//			| (info.utc.min << 5)       // Min =  
//			| (info.utc.sec >> 1)       // Sec =  
//		
    rtc_gettime(&rtc);	                      
	return    ((rtc.year - 60) << 25)       // Year ()  60 debuga
			| (rtc.month << 21)       // Month = 
			| (rtc.mday << 16)       // Day =  
			| (rtc.hour << 11)       // Hour 
			| (rtc.min << 5)       // Min =  
...
/*******************************************************************************
* Function Name  : rtcCorrectTimeFromGLONASS
* Description    : 
* Input          : None
* Output         : None
* Return         : not used
...
{
  rtc->sec   = infoPtr->utc.sec;
  rtc->min   = infoPtr->utc.min;
  rtc->hour  = infoPtr->utc.hour;
  rtc->mday  = infoPtr->utc.day;
  rtc->month = infoPtr->utc.mon+1;
  rtc->year  = infoPtr->utc.year+1900;
  rtc_settime(rtc);	
}
...
u32 rtcGetNaviTime ( nmeaINFO * infoPtr )
{
  ...
  return    ((infoPtr->utc.year-100)  << 26)       // Year since 2000
			| (infoPtr->utc.mon+1 << 22)       // Month = 
			| (infoPtr->utc.day   << 17)       // Day =  
			| (infoPtr->utc.hour  << 12)       // Hour 
			| (infoPtr->utc.min   << 6)       // Min =  
			| (infoPtr->utc.sec )       // Sec =  
			                     ;
}
/*******************************************************************************/
...
{
  rtc_gettime(&rtc);	
  return    ( (rtc.year - 2000) << 26)       // Year ()  60 debuga
			| (rtc.month      << 22)       // Month = 
			| (rtc.mday       << 17)       // Day =  
			| (rtc.hour       << 12)       // Hour 
			| (rtc.min        << 6)       // Min =  
