...
void nmea_GPGSA2info(nmeaGPGSA *pack, nmeaINFO *info)
{
    ...
    info->VDOP = pack->VDOP;
   	...
   	info->satinfo.inuseGLN = 0;   //KAY
	info->satinfo.inuseGPS = 0;	  //KAY
	info->satinfo.inuse = 0;	  //KAY
 #if 1
    for(i = 0; i < NMEA_MAXSAT; ++i)
    {
        for(j = 0; j < info->satinfo.inview; ++j)
        {
            if(pack->sat_prn[i] && pack->sat_prn[i] == info->satinfo.sat[j].id)  //was ini
            //if(pack->sat_prn[i])	   //test 
            {
                info->satinfo.sat[j].in_use = 1;
				if(pack->sat_prn[i] < 33)
                    info->satinfo.inuseGPS++;
				else if(pack->sat_prn[i] > 64)
                    info->satinfo.inuseGLN++;
            }
        }
    }
#endif
        ...
        for(j = 0; j < 12; ++j)
        {
            if(pack->sat_prn[j])	   //test 
            {
                info->satinfo.sat[j].in_use = 1;
				if(pack->sat_prn[j] < 33)
                    info->satinfo.inuseGPS++;
				else if(pack->sat_prn[j] > 64)
                    info->satinfo.inuseGLN++;
            }
        }
#endif
	//info->satinfo.inuseGLN = 5;   //KAY	testing
    info->smask |= GPGSA;
	//KAY
	#ifndef BRIDGE_USB_GSM
		        //USBWriteStr("GSA: ");
